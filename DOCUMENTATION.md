...
A standalone summary page for policymakers. All logic is self-contained in `five-year-change/index.html` (inline JS, no shared modules). Key sections:
...
The project uses [Playwright](https://playwright.dev/) smoke tests to catch regressions before production.
...
Tests run against a local static server on port 8765.
