...
│   ├── og-change-summary.png # OG image shared across all 5 Change Summary views
...
Contains data for Hawaiʻi's 4 counties (Honolulu, Hawaiʻi County, Maui, Kauai) for metrics where county-level federal data is available. Used for the **County** tab in the modal.
...
3. **Policy Area Overview scorecard** - one row per area, sorted strong → mixed → weak. Left-border color and "X of Y above avg." text are both colored from the same signal: green (≥60% of the area's metrics above national median), yellow (mixed, 40-60%), red (≤40%). Trend arrows at right show improving/worsening counts.
4. **Area sections** - collapsed by default; `Utils.generateAreaNarrative()` produces a 5-sentence summary per area (trend mix, ranking moves, median standing, quartile callout). Expand toggle reveals individual metric rows.
