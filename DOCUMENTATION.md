...
│   ├── fyc.css             # Change Summary page styles (all 7 views)
...
│   ├── fonts/              # Bundled TTFs for OG card rendering (Liberation Sans, SIL OFL 1.1)
...
│   ├── og_font.py             # Shared font resolver for all OG generators (bundled Liberation Sans first → Mac and CI render identically)
...
| OG image generation | Python 3 + Pillow (PIL), bundled Liberation Sans (assets/fonts/) |
...
Generated by `scripts/build-county-data.js` (all 11 metrics sourced from federal APIs). Currently covers 11 metrics:
...
5. **Two comparison sections:**
   - **vs Other States** - "Better" (green) or "Worse" (red) with the median, plus a meta row showing:
     ...
     - **By county**: small muted teal link, only present for the 11 metrics with `COUNTY_DATA`. Clicking opens the modal directly on the County tab.
...
| Variable | Hex | Usage |
|----------|-----|-------|
| `--hawaii-blue` | `#0C7081` | Links, UI accents, area labels, buttons (the chart Hawaiʻi line uses the `#0D7C8F` literal in charts.js) |
| `--hawaii-blue-light` | `#EEF8FA` | Hint/callout box backgrounds |
| `--positive` | `#065F46` | "Better" / "Improving" |
| `--positive-bg` | `rgba(5, 150, 105, 0.10)` | Background for positive chips |
| `--negative` | `#991B1B` | "Worse" / "Worsening" |
| `--negative-bg` | `rgba(192, 57, 43, 0.10)` | Background for negative chips |
| `--neutral` | `#8f6700` | "Little change" / amber |
...
| `scripts/audit-narrative-numbers.js` | **Drift scanner** (data.js + questions.js). Verifies every quantitative claim in every narrative field against the underlying time series. `--gate --gate-new` is wired into `npm run validate`; `--gate-new` covers the county-value, superlative, rank-window and 50-state-quantifier shapes, blocking since 2026-08-09. Rank-window claims honour a named exception (`"…since 2004 except 2020"`), so disclosing an outlier does not read as a violation. Rank claims are checked in `rankHistoryNarrative.explore[]` as well as `.summary`, but only in sentences naming Hawaiʻi and no other state, since `explore[]` discusses comparator states in the same field. Ranks must match exactly, except that a claim anywhere in a genuine tie block passes (the order inside a tie is arbitrary), and a band whose edge this parser inferred rather than the prose stating it ("near last", "top quarter") keeps one rank of give. |
...
- Links inside post body: inherit global teal (`#0C7081`); add 1px underline + 2px offset via `.otc-post a`.
...
`rainy-day-fund-rule`, `low-violent-crime-high-homelessness`, `productivity-vs-unemployment`, `reading-without-raising`, `expensive-states`, `renewables-prices` (newest first).
...
| `/c/{slug}/` (27 pages) | Dataset (with `temporalCoverage`, `spatialCoverage`, `variableMeasured`, `publisher`, and `distribution` referencing `/data/{slug}_state.csv` + `/data/{slug}_county.csv` when applicable) + BreadcrumbList |
| `/about/` | AboutPage + VideoObject (the tour) + BreadcrumbList |
...
`/data/{slug}_state.csv` (one per metric, 27 files) and `/data/{slug}_county.csv` (one per metric with county data, 11 files) are pre-generated by `scripts/generate-og-pages.py` in long format (`year, state, value` and `year, county, value` respectively). They're served with `Content-Type: text/csv; charset=utf-8` and `Access-Control-Allow-Origin: *` so they can be fetched cross-origin (Python pandas, R, Datasette, etc.). The schema.org `Dataset.distribution` on each `/c/{slug}/` declares them so Google Dataset Search and AI crawlers can discover the underlying data, not just the rendered page.
