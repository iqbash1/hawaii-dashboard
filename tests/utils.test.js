...
// ----------------------------------------------------------------
// Utils.renderCountyLine
// ----------------------------------------------------------------
describe('Utils.renderCountyLine', () => {
    it('returns empty string for null input', () => {
        assert.equal(Utils.renderCountyLine(null), '');
    });
    it('returns compressed text wrapped in fyc-line4 div', () => {
        const html = Utils.renderCountyLine({ compressed: true, text: 'Counties: all 4 improved' });
        assert.equal(html, '<div class="fyc-line4">Counties: all 4 improved</div>');
    });
    it('renders improved county with up arrow and county-improved class', () => {
        const html = Utils.renderCountyLine({
            compressed: false,
            counties: [{ county: 'Honolulu', direction: 'improved' }],
        });
        assert.ok(html.includes('county-improved'), 'should have county-improved class');
        assert.ok(html.includes('Honolulu'), 'should include county name');
        assert.ok(html.includes('\u2191'), 'should include up arrow');
    });
    it('renders worsened county with down arrow and county-worsened class', () => {
        const html = Utils.renderCountyLine({
            compressed: false,
            counties: [{ county: 'Maui', direction: 'worsened' }],
        });
        assert.ok(html.includes('county-worsened'));
        assert.ok(html.includes('\u2193'));
    });
    it('renders flat county with right arrow and county-flat class', () => {
        const html = Utils.renderCountyLine({
            compressed: false,
            counties: [{ county: 'Kauai', direction: 'flat' }],
        });
        assert.ok(html.includes('county-flat'));
        assert.ok(html.includes('\u2192'));
    });
    it('joins multiple counties with middle dot separator', () => {
        const html = Utils.renderCountyLine({
            compressed: false,
            counties: [
                { county: 'Honolulu', direction: 'improved' },
                { county: 'Maui', direction: 'worsened' },
            ],
        });
        assert.ok(html.includes('\u00b7'), 'should separate counties with middle dot');
        assert.ok(html.includes('Honolulu'));
        assert.ok(html.includes('Maui'));
    });
});
