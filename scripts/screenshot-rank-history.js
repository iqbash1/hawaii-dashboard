#!/usr/bin/env node
// Generate OG images for /rh/{slug}/ rank-history tab.
// Output: assets/og/{slug}_rank_history.png  (1200x630)
// Usage: node scripts/screenshot-rank-history.js [slug1 slug2 ...]
...
function startServer(port) {
    return new Promise((resolve) => {
        const server = http.createServer((req, res) => {
            // Serve static files; for any /rh/ path serve index.html
            const url = req.url.split('?')[0];
            if (url.startsWith('/rh/') || url === '/') {
                req.url = '/index.html';
            }
...
async function screenshot(page, slug) {
    const url = `http://localhost:7788/#${slug}/rank-history`;
    await page.goto(url, { waitUntil: 'networkidle0', timeout: 30000 });

    // Wait for rank-history panel to be visible
    await page.waitForSelector('#modal-rank-history', { visible: true, timeout: 15000 });

    // Wait for canvas to be drawn (rank-history-chart should have non-zero height)
    await page.waitForFunction(() => {
        const c = document.getElementById('rank-history-chart');
    ...
    const outPath = path.join(OUT_DIR, `${slug}_rank_history.png`);
    fs.writeFileSync(outPath, buf);
    console.log(`  ✓ ${slug}_rank_history.png`);
}
...
async function main() {
    ...
    console.log(`Generating ${targets.length} rank-history OG images...`);
    ...
    if (ok > 0) {
        console.log('\nNext: update rt/ stubs to reference _rank_history.png images, then commit.');
    }
}
