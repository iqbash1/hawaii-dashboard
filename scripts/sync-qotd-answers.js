 ...
 *   V7    (vs named state) -- comparator state parsed from chartUrl /rh/{slug}/{code}/.
...
// Map state-code from /rh/{slug}/{code}/ to canonical state name.
const STATE_CODES = {
...
function renderHiVsState(q, metric) {
    // Pull state code from chartUrl: /rh/{slug}/{code}/
    const m = (q.chartUrl || '').match(/^\/rh\/[^/]+\/([a-z]{2})\/?$/i);
    if (!m) return null;
