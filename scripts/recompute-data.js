...
// ==========================================================
// Step 1: Drop ACGR years that came through only partially
// ==========================================================
//
...
//
// This step used to delete years 2023 and 2024 by name, on the reasoning that
// d23 stops at school year 2021-22 so anything later must have leaked in. That
// was a ceiling, not a guard: recompute runs AFTER the fetcher in
// refresh-data.yml, so once NCES ships an edition carrying 2022-23, the
// fetcher would read it correctly and this function would delete it moments
// later, leaving the series pinned at 2022 and still looking healthy.
//
// Test coverage instead of year numbers. A real edition year carries close to
// every state (existing years run 47-50); a leak carries a handful. That keeps
// the original intent, catching a partial year, without capping the series.

const ACGR_MIN_STATES = 40;
...
function fixACGR() {
    ...
    for (const [year, row] of Object.entries(acgr.data)) {
        const n = Object.keys(row).length;
        if (n < ACGR_MIN_STATES) {
            delete acgr.data[year];
            fixes.push(`Removed ACGR ${year} (only ${n} states, expected >= ${ACGR_MIN_STATES})`);
        }
    }
...
// Banner text is kept identical to the one build-state-data.js writes. Both
// scripts write js/state-data.js (build fetches, then recompute mutates and
// rewrites), so differing banners made the file's header flap between the two
// and dropped build's provenance note from whatever landed in git.
